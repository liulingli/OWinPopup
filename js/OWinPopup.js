		...
		init: function(options) {
			return this.each(function() {
				...
				var defaluts = {
					...
					url: '', //弹出窗口地址
					title: '', //弹出窗口标题
  					titleBgColor:'#04bfea', //弹出窗口标题背景颜色
					titleIconSrc: 'img/ppicon-white.png', //弹出窗口标题图标地址
					footBtn: 'type1', //底部显示按钮，'type1'：显示确认按钮;'type2'显示取消和确认按钮；'type3'不显示按钮
					width: 300, //弹出窗口宽度
					height: 150, //弹出窗口高度
					isHasOverlay: true, //弹出窗口是否需要遮罩，需要为true,不需要为false
					resizeMinW: 300, //弹出窗口最小缩放宽度
					resizeMinH: 200, //弹出窗口最小缩放高度
			...
			return this.each(function() {
				...
				var RULES = {
					...
					checkConfig: function(data) {
						...
						return _setting.srcPath + _setting.url;
					},
					creatHtml: function() {
						var winW = $this.width(),
							winH = $this.height(),
						...
						var iframeUrl = RULES.checkConfig(_setting.configParam); //将请求参数赋到地址后面
           				var iframeH = btnHtml==''?(_setting.height-21):(_setting.height-51);
						var overlayHtml = "<div class='overlay' id='overlay" + _setting.zIndex + "'" +
							"style='z-index:" + (_setting.zIndex + 1) + "'></div>"
						var popWinHtml = "<div id='popWinContent" + _setting.zIndex + "' class='popWin' style='width:" + _setting.width + "px;height:" + _setting.height + "px;z-index:" + (_setting.zIndex + 2) + ";" +
							...
							"<div id='popWinHead" + _setting.zIndex + "' style='background:"+_setting.titleBgColor+";' class='popWinHead'>" +
							...
							"<b style='color:red;'>×</b></a></span></div>" +
							"<div class='Hdiv'></div><img id='loadingSpan" + _setting.zIndex + "' src='" + _setting.srcPath + "img/loading.gif'/>" +
							"<iframe name='popWinFrame" + _setting.zIndex + "' id='popWinFrame" + _setting.zIndex + "' class='popWinFrame' style='height:"+iframeH+"px;'"+
							"src='" + iframeUrl + "'frameborder='no' border='0' marginwidth='0' marginheight='0' scrolling='yes' allowtransparency='yes'>" +
					...
					winDrag: function() {
						...
						ohandle.on("mousedown.winDrag", function(event) { //鼠标按下事件
							...
							$this.on("mousemove.winDrag", function(event) { //鼠标移动事件
								$(".Hdiv",$this).css("display", "block"); //该元素用于解决iframe框架下获取不了事件的bug
								var event = event || window.top.event,
									isL = event.clientX - disX,
									isT = event.clientY - disY;
								maxL = $(window.top).width() - oDrag.width()-8;
								maxT = $(window.top).height() - oDrag.height()-8;
								//碰撞检测
								isL <= 8 && (isL = 8);
								isT <= 8 && (isT = 8);
								isL >= maxL && (isL = maxL);
							...
							$this.on("mouseup.winDrag", function() { //鼠标释放事件
								$(".Hdiv",$this).css("display", "none"); //该元素用于解决iframe框架下获取不了事件的bug
					...
					winResize: function() { //弹出窗口可改变大小
						window.top.resizeAll = function(oParent, handle, isLeft, isTop, lockX, lockY) {
							handle.on("mousedown.winResize", function(event) {
								var event = event || window.top.event,
								  left = isLeft?handle.css("left"):oParent.width()-handle.width(),
								  top= isTop?handle.css("top"):oParent.height()-handle.height(),
									disX = event.clientX - parseFloat(left),
									disY = event.clientY - parseFloat(top),
								...
								$this.on("mousemove.winResize", function(event) {
									...
									lockY || (oParent.css("height", iHeight)&&$(oParent).find("iframe").css("height", (_setting.footBtn =='type3'?(iHeight-21):(iHeight-51))));
									...
									$(".Hdiv",$this).css("display", "none"); //清除遮罩
				...
				$this.on("click.win", "#popWinClose" + _setting.zIndex + ",#cancelBtn" + _setting.zIndex + "", function() {
					...
					//解绑事件
					$this.off("click.win", "#popWinClose" + _setting.zIndex + ",#cancelBtn" + _setting.zIndex);
				...
				$("#popWinFrame" + _setting.zIndex, $this).load(function() {
					...
					var iframeID = Event.iframeLoad(_setting.zIndex).iframeID,
						topIframe = Event.iframeLoad(_setting.zIndex).topIframe,
						winPopupIframe = Event.iframeLoad(_setting.zIndex).winPopupIframe;
					_setting.openFun(topIframe,winPopupIframe,iframeID); //执行加载完成函数	            	 
					//解绑事件
